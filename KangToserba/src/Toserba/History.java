...
public static void tampilkanTransaksi() {
        String [] judul = {"No Transaksi","Tanggal","Total Harga","ID Karyawan"};
        model = new DefaultTableModel(judul,0);
        tb_transaksi.setModel(model);
        
        try{
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_toserba","root","");
            ResultSet rs = con.createStatement().executeQuery("SELECT * FROM tb_penjualan ");
            while(rs.next()){
    ...
    private void initComponents() {
        ...
        getContentPane().add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(400, 80, 390, 80));
    ...
    private void tb_transaksiMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tb_transaksiMouseClicked
        ...
        try{
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_toserba","root","");
            ResultSet rs = con.createStatement().executeQuery("SELECT tb_detail_penjualan.*,tb_barang.nama_barang, tb_barang.harga_jual FROM tb_detail_penjualan JOIN tb_barang ON tb_detail_penjualan.id_barang=tb_barang.id_barang WHERE tb_detail_penjualan.id_penjualan='"+transaksi+"'");
            while(rs.next()){
                String data [] = {rs.getString("nama_barang"),rs.getString("harga_jual"),rs.getString("tb_detail_penjualan.jumlah")};
                model.addRow(data);
            }
        }
        catch (SQLException ex){
    ...
    private void btn_submitMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btn_submitMouseClicked
        ...
        try{
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_toserba","root","");
            ResultSet rs = con.createStatement().executeQuery("SELECT * FROM tb_penjualan WHERE tgl_penjualan BETWEEN '"+tanggaldari+"' AND DATE_ADD('"+tanggalsampai+"',INTERVAL 1 DAY)");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try {
          ...
          try {
              String report = ("C:\\Users\\Ariz\\Downloads\\KangToserba\\src\\Report\\report1.jrxml");
              MashMap hash = new MashMap ();
              hash.put("kode",tb_transaksi.getText() );
